...
If GitHub, release preview, or version tooling is unavailable, stop before
publishing and record the missing command. Do not infer remote PR state from
local branch names alone.
...
If `gh` is unavailable or unauthenticated, ask for remote PR context before
