...
If baseline timing or artifacts are missing, capture a fresh baseline before
optimizing. Do not claim performance improvement from intuition alone.
...
local metrics.
If an advisory command is unavailable, continue with shell timing, test logs,
build output, and source-level bottleneck inspection.
