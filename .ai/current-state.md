...
- the continuous research-loop integration now has a bounded evidence-only executor: `research-cycle-plan` exposes PRE-FLIGHT, MONITOR, ANALYZE, PROPOSE, and DIGEST phase intent, while `research-cycle-run` can run one or more sidecar collection cycles, persist snapshot records, and report safety policy without broker access, proposal approval, proposal creation, or raw web prompt injection
- `agentic_trader.system.tool_roots` now owns the first central registry for optional repo tools, including status IDs, consumers, fallback order, manifests, and safe status notes for Ollama, Firecrawl, and Camofox; setup-status consumes this registry, while deeper Web/Ink/research/QA parity is still an incremental follow-up
...
- `.ai/skills/ruflo-codex.md` and `.ai/playbooks/ruflo-advisory-checks.md` now document MCP-first plus global-CLI RuFlo usage for routing, diff risk, command risk, security, performance, coverage-gap, dependency, and code-boundary review; repo-local RuFlo init/state remains out of scope
