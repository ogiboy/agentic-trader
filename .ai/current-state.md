...
- the continuous research-loop integration is currently contract-first and read-only: `research-cycle-plan` exposes PRE-FLIGHT, MONITOR, ANALYZE, PROPOSE, and DIGEST phase intent, but no autonomous loop executor exists yet; sidecar evidence remains normalized/provenance-only, proposal creation stays reviewable, and broker/policy mutation remains blocked
