...
- `agentic_trader.system.tool_roots` now owns the first central registry for optional repo tools, including status IDs, consumers, fallback order, manifests, and safe status notes for Ollama, Firecrawl, and Camofox; setup-status, model-service status, Camofox-service status, and research provider metadata now consume the shared registry, while deeper Web/Ink/QA parity is still an incremental follow-up
