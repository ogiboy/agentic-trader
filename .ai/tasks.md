...
- add a proposal reconciliation follow-up before external paper/live adapters grow broader: if a process exits after broker submission but before the final proposal status update, the idempotent `execution_records.intent_id` upsert should let a repair command mark the proposal terminal without resubmitting the order
...
- implement the continuous research loop in phases after the read-only plan contract is stable: PRE-FLIGHT must verify provider/tool/source health, MONITOR must collect source-attributed events with cadence/pause/resume metadata, ANALYZE must materialize normalized evidence summaries without raw web text, PROPOSE must create only pending review-queue candidates, and DIGEST must write reviewable snapshot links plus memory packets after redaction/staleness checks
...
- use `.ai/agents/market-strategist.md`, `.ai/strategies/v1-strategy-catalog.md`, and `.ai/playbooks/strategy-research-and-sweeps.md` when adding scanner presets, strategy-family hints, proposal enrichment, backtest baselines, or sweep plans
- use `.ai/playbooks/news-intelligence.md` and `.ai/skills/market-news-research.md` when adding ticker/news/macro/browser evidence so source tier, fetcher source, attempts, freshness, materiality, and redaction survive into snapshots and proposal notes
- use `.ai/playbooks/finance-evidence-reconciliation.md` when broker/accounting/reporting changes touch account truth, fills, fees, slippage, PnL, exposure, or evidence-bundle payloads
...
- extend the first `tool_roots` helper into a central local-tool readiness contract before deeper runtime integration: setup-status, model-service, camofox-service, researchd, WebGUI, QA, and docs should consume the same tool truth for Camofox, Ollama, Firecrawl, RuFlo, Docker, and future browser/fetcher fallbacks
