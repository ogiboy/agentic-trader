...
- keep maturing the bounded continuous research loop: `research-cycle-run` now executes sidecar collection cycles and snapshot persistence without broker/proposal authority; next passes should add explicit PRE-FLIGHT gate payloads, pause/resume/trigger-now state, source-health deltas, cadence status, and digest artifacts before any daemonized executor
...
- keep extending the central local-tool readiness registry: setup-status now consumes `agentic_trader.system.tool_roots` definitions for Ollama, Firecrawl, and Camofox status IDs, consumers, fallback order, manifests, and safe notes; next passes should route model-service, camofox-service, researchd, WebGUI, QA, docs, RuFlo, Docker, and future browser/fetcher fallbacks through the same truth instead of re-probing independently
