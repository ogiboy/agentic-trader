...
If a required local server, route, CLI JSON command, or browser tool is
unavailable, record the missing dependency and continue with the strongest
remaining evidence. Do not mark the scenario passed from screenshots alone.
...
5. Advisory checks, only when RuFlo is already available and stable:
