...
7. Run a backtest or comparison only after the data window and lookahead rules
   are explicit.
8. Before launching a sweep, define the wall-time limit, stop condition,
   artifact path, and mid-run failure behavior.
9. If a backtest, confidence review, or no-lookahead check fails, record the
   failed gate and keep the idea in research or watchlist status.
10. If the idea can act, create a pending proposal; do not execute.
...
If a sweep exceeds its wall-time estimate or fails mid-run, stop the job when
safe, preserve partial artifacts, record completed parameter sets, and mark the
candidate as inconclusive rather than proposal-ready.
...
If a strategy fails backtests, confidence review, or no-lookahead checks,
document the failed gate, likely cause, and next research adjustment. Do not
silently tune parameters until it passes.
