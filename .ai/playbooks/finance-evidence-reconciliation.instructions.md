...
If a required broker, account, storage, or provider artifact is missing, mark
the affected fields unavailable and identify the missing artifact. Never infer a
clean zero, successful fill, or healthy broker state from absence of evidence.
...
If two surfaces disagree, treat persisted broker/accounting records and adapter
payloads as the evidence to inspect first, then repair the stale display or
document the unresolved mismatch. Do not make the UI number authoritative by
editing only labels or docs.
