...
If a provider, browser helper, API key, SDK, or sidecar is missing, record that
as source-health evidence and continue only with enabled sources. Missing
evidence lowers confidence; it must not be silently converted into neutral or
supportive evidence.
...
Every news event or article summary that is allowed to influence a scanner,
proposal, or operator digest must validate against the code-owned
...
When a fetcher chain is used, preserve each attempt in this order:
...
When implementing providers or sidecar flows, add tests for:
