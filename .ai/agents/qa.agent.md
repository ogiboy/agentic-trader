...
Also read the shared repository context listed in `.ai/agents/README.instructions.md`.
...
- Pull in `.ai/agents/operator-ux.agent.md` when the task involves visual layout,
  command clarity, menu navigation, resize behavior, or finance/accounting
  readability.
- Pull in `.ai/agents/finance-ops.agent.md` when the task changes broker, account,
  PnL, exposure, order lifecycle, or execution audit behavior.
- Pull in `.ai/agents/product-docs.agent.md` when a feature should be explained to an
  end user or operator, not only to a contributor.
- Pull in `.ai/workflows/qa-workflow.instructions.md` for tier selection and
  `.ai/playbooks/browser-qa.instructions.md` for WebGUI/docs browser checks.
