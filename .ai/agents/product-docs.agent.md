...
Start with the shared role contract in `.ai/agents/README.instructions.md`, then read:
...
If code behavior, runtime status, or QA evidence cannot be verified, label the
docs change as proposed or contributor-facing. Do not write operator docs that
claim a feature is ready solely because it appears in a roadmap or `.ai` plan.
