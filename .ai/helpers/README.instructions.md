...
If a referenced helper, external tool, or root script is unavailable, record the
missing dependency and use the documented repo-native fallback instead of
adding generated helper state.
...
| Setup validation           | Use `.ai/playbooks/setup-validation.instructions.md` and root pnpm/uv scripts.                                              |
| Security scan              | Use `.ai/playbooks/security-scan.instructions.md`, Sonar/CodeRabbit findings, focused negative tests, and redaction checks. |
| Performance benchmark      | Use `.ai/workflows/performance-workflow.instructions.md` with measured setup/check/build/runtime timings.                   |
| Diff-risk advisory         | Use RuFlo MCP diff-risk/file-risk tools when available, then verify locally.                                                |
| GitHub safety              | Use `.ai/workflows/release-pr-workflow.instructions.md` for base, version, PR, and release checks.                          |
| Browser QA                 | Use `.ai/playbooks/browser-qa.instructions.md` and the Browser/Computer tools when available.                               |
