...
- the Web GUI control room now has a clearer modular/i18n seam: domain views live in `webgui/src/components/control-room/`, shared panel/list/JSON primitives are separate, request/auth helpers live in `api.ts`, dashboard polling lives in `dashboard-polling.ts`, mutation request helpers live in `action-request.ts`, runtime/tool/proposal/chat/instruction actions live in `actions.ts`, locale/loading state hooks live in `state-hooks.ts`, shell chrome lives in `shell.tsx`, typed view-model builders live in `view-model.ts`, helper-generated diagnostics/context evidence lines live in dedicated formatting modules, and shell/auth/overview/runtime/portfolio/proposal/review/memory/chat/settings plus overview readiness/provider/local-tool diagnostic copy lives in typed per-locale modules under `control-room/copy/` with a small `labels.ts` facade and persisted local language choice after hydration
...
- pyright is now configured as a first-class static check for repository source, tests, and QA scripts; strict Pyright/Pylance mode currently exposes a large staged-hardening backlog, so source/runtime risks should be fixed before broad test/mock unknown-type noise; the first type-stub pass added `pandas-stubs` plus local `typings/yfinance` and `typings/firecrawl` shims, removing missing-stub diagnostics, and the first test/mock passes brought `tests/test_setup_status.py`, `tests/test_app_lifecycle.py`, `tests/test_broker_adapter.py`, `tests/test_config.py`, `tests/test_position_manager.py`, `tests/test_llm_client.py`, `tests/test_trade_proposals.py`, and `tests/test_data_providers.py` to 0 Pyright errors without loosening strict mode; `tests/test_runtime_tools.py` is partially reduced but still needs public model-service diagnostic seams; the ProviderSet default-factory source diagnostics are also clean; the full strict backlog is now 1776 errors after the data-provider/provider-set pass, and CI/release/smoke quality checks use the strict baseline gate so new regressions fail while old debt remains visible
...
- V1 is not yet a revenue-ready commercial trading product: before paid users or live-money claims, the project needs a classified compliance model, explicit Alpaca production/account-opening responsibilities, risk disclosures and terms, audit/data-retention exports, support/incident posture, stale/degraded UX proof, and per-cycle remote-model cost telemetry; the first monetizable path should be local-first paper desk and evidence-bundle value, not managed live execution
