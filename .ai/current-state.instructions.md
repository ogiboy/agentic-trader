...
- the Web GUI control room now has a clearer modular/i18n seam: domain views live in `webgui/src/components/control-room/`, shared panel/list/JSON primitives are separate, request/auth helpers live in `api.ts`, dashboard polling lives in `dashboard-polling.ts`, runtime/tool/proposal/chat/instruction actions live in `actions.ts`, shell chrome lives in `shell.tsx`, and shell/auth/overview/runtime/portfolio/proposal/review/memory/chat/settings copy lives in typed per-locale modules under `control-room/copy/` with a small `labels.ts` facade and persisted local language choice
...
- pyright is now configured as a first-class static check for repository source, tests, and QA scripts; strict Pyright/Pylance mode currently exposes a large staged-hardening backlog, so source/runtime risks should be fixed before broad test/mock unknown-type noise
...
- `webgui/src/components/control-room.tsx` is now reduced to control-room state wiring and render composition; the next maintainability pass should continue moving remaining global CSS toward screen-scoped components and move helper-generated evidence lines/view models behind catalog-aware helpers before adding broad new Web GUI behavior
