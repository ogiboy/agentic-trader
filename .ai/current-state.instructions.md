...
- provider adapter foundation with Ollama behind a provider boundary plus an explicit OpenAI-compatible adapter escape hatch for operator-supplied model endpoints
...
- the Web GUI Local Tools panel now exposes the internal-first helper contract: app-owned local tools, explicit host fallback ownership, app-owned Ollama/qwen status from dashboard/doctor truth, and access-key-gated Camofox starts
- the Web GUI control room now has a clearer modular/i18n seam: `webgui/src/components/ControlRoom.tsx` is the state wiring component, domain views live in PascalCase files under `webgui/src/components/control-room/`, shared panel/list/JSON primitives are separate, request/auth helpers live in `api.ts`, dashboard polling lives in `dashboard-polling.ts`, mutation request helpers live in `action-request.ts`, runtime/tool/proposal/chat/instruction actions live in `actions.ts`, locale/loading state hooks live in `state-hooks.ts`, shell chrome lives in `Shell.tsx`, typed view-model builders live in `view-model.ts`, helper-generated diagnostics/context evidence lines live in dedicated formatting modules, and WebGUI copy is exposed through a `next-intl` provider/request/routing layer with a compatibility bridge from the typed per-locale control-room copy modules
...
- the docs app now uses locale-prefixed English and Turkish routes (`/en/...` and `/tr/...`) with localized page trees, localized feedback copy, PascalCase React component files, and a modular frontend split across home, feedback, layout, `components/ui/primitives`, i18n, and content helpers instead of one overloaded docs page file
...
- pyright is now configured as a first-class static check for repository source, tests, QA scripts, and the tracked CrewAI sidecar source; strict Pyright/Pylance is clean at zero diagnostics for `agentic_trader`, `tests`, `scripts`, and `sidecars/research_flow/src`, no `type: ignore` or Pyright suppression comments remain in the repository, and CI/release/smoke quality checks now use the strict zero-diagnostic gate rather than a temporary backlog baseline
...
- root daily development and GitHub Actions now default to Python 3.13 through the repo-managed `.python-version`; the package metadata still allows `>=3.12,<3.15`, but 3.12 compatibility is no longer the primary CI lane unless a separate matrix is added
...
- recurring operator-facing labels and prompts now flow through a typed shared Python UI text catalog with English/Turkish entries and legacy constants kept for existing CLI/Rich/Ink imports, giving future localization a safer boundary without forcing a full rewrite
...
- `research-flow-setup` reports the tracked `research-flow` workspace dependency path, Python version file, root lockfile/workspace environment presence, uv availability, and root setup/check/run commands; CrewAI is still not imported by core runtime modules and remains subprocess-only at runtime
- the CrewAI research backend now uses a subprocess JSON contract through `uv run --package research-flow --locked --no-sync research-flow-contract` when the workspace environment is installed; failures stay visible and do not install dependencies implicitly during runtime
...
- `pnpm run app:doctor` is now the first lifecycle facade: it resolves an already installed `agentic-trader` entrypoint without invoking `uv run`, then reads setup, app-owned service, provider-diagnostics, and network-light V1 readiness payloads without installing dependencies, starting/stopping services, pulling models, opening browsers, or starting a trading daemon; human output now summarizes degraded setup/service/provider/V1 facts instead of showing only step-level ok/fail
- `pnpm run app:setup` is now the first conservative setup lifecycle facade: it defaults to dry-run planning, and the only mutating path is explicit `--core --yes` repair that runs the existing root Node workspace setup plus root uv Python sync; sidecar setup, Camofox browser fetches, model pulls, app-owned service starts, Web GUI launch, update, and uninstall remain separate opt-in slices
- `pnpm run app:start` and `pnpm run app:stop` now cover the first conservative app-owned service lifecycle slice: both default to dry-run, require selecting `--webgui`, `--model-service`, `--camofox-service`, or `--all` plus `--yes` before mutation, call the existing service commands with JSON output, keep Web GUI browser opening opt-in, print service-scope-specific next commands in dry-run output, and do not install dependencies, fetch browsers, pull models, or start a trading daemon; WebGUI-service stop preserves app-owned state for retry if the recorded process cannot be stopped
- `pnpm run app:update` now provides the first scoped update lifecycle lane: dry-run by default, explicit `--core`, `--sidecar`, `--camofox`, `--build`, `--status`, or `--all` plus `--yes` before mutation, native owners for root pnpm, root uv, sidecar uv, and Camofox pnpm updates, and no browser binary fetch, model pull, service start/stop, trading daemon, secrets, brokerage config, or runtime-state deletion
- `pnpm run app:uninstall` now provides the first conservative uninstall lifecycle lane: dry-run by default, explicit `--artifacts`, `--deps`, `--service-state`, or `--all` plus `--yes` before removal, generated/dependency/service-log scopes only, and service-state cleanup blocks while recorded service state files remain
- `pnpm run app:up` now provides the first guided first-run orchestration layer: dry-run by default, safe `--all` composition for core repair, CrewAI Flow sidecar setup, app-owned Web GUI start, and final doctor, plus explicit ownership flags before optional Camofox dependency/browser lanes or model/Camofox service starts; it still does not pull models, fetch browsers, create provider accounts, touch broker config, or start a trading daemon unless the operator selects the matching explicit scopes
...
- `make bootstrap` now reuses persisted Ollama/Firecrawl/Camofox ownership instead of re-prompting every launch, and its summary prints explicit why/next lines for deferred setup work such as PATH entrypoints, Camofox dependencies, and browser binary fetches
- `setup-status` keeps optional CrewAI global CLI failures bounded: a broken host `crewai --version` is a setup note, not a multi-line version string or traceback in operator JSON
...
- `tools/camofox-browser` depends on `camoufox-js` as the expected Node.js Camoufox bridge/fetch CLI; bootstrap output and helper docs should say this explicitly so the package name does not look like an unrelated install
...
- V1 is not yet a revenue-ready commercial trading product: the type-safety branch research refresh keeps paid access and live-money claims blocked until the project has a classified compliance model, explicit Alpaca production/account-opening responsibilities, risk disclosures and terms, privacy/data-retention controls, audit exports, support/incident posture, stale/degraded UX proof, LLM/tool-poisoning controls, and per-cycle remote-model cost telemetry; the first monetizable path should be a local-first paper desk, evidence-bundle export, operator education, and personal automation value, not managed live execution, copy trading, performance-fee claims, or account-opening workflows
...
- CrewAI is not a trading-runtime authority; the CrewAI backend is an isolated placeholder and must not replace the staged specialist graph. The tracked Flow scaffold now lives under `sidecars/research_flow/`, outside `agentic_trader/`, as the root uv workspace package `research-flow` with its version aligned to the root app version.
...
- the docs surface now supports English and Turkish locale routes, and the broader CLI/Rich/Ink/Web GUI localization track is active as staged foundation work rather than a one-shot rewrite; WebGUI now uses a `next-intl` accessor/provider foundation, Python terminal copy has a shared `ui_text.t("namespace.key")` facade, Ink TUI navigation/status/index-level copy honors `AGENTIC_TRADER_UI_LOCALE`, and dashboard/observer JSON keys must remain stable English contract keys
- the Python CLI entrypoint is now a Typer registration/compatibility facade with dashboard, registration, callback, command, and rendering ownership pushed into focused `agentic_trader/cli_modules/` modules; command/rendering modules no longer use the broad `from agentic_trader import ui_text as text` import shape, and global `--locale` overrides are scoped to the current Typer invocation so same-process tests do not leak UI locale
- the next i18n cleanup should continue reducing heavy copy imports: WebGUI should migrate remaining compatibility catalog access to `useTranslations(...)` by screen, Ink TUI page-panel copy should move behind the same TUI copy catalog, and remaining Python terminal modules should migrate explicit constant imports toward the shared locale-aware `t("namespace.key")` accessor instead of importing large label tables into command or component files
...
- `.codex/environments/environment.toml` setup, check, and preview actions now include the tracked CrewAI Flow sidecar setup/check commands plus lifecycle dry-runs for app:doctor/setup/start/stop/update/uninstall so Codex workspace actions do not drift from README and root pnpm scripts
...
- stable release changelog generation now also guards against feature-branch prerelease tags consuming all release-history entries before the final `main` release: if semantic-release leaves the stable section empty, the workflow backfills it from the previous stable tag to `HEAD`, ignoring prerelease tags
...
- The project now intentionally tracks a local advisory skill/catalog layer: `.agents/skills/`, `skills-lock.json`, `CLAUDE.md`, `.claude/`, and stable `.claude-flow/` config/capability files. These files align Claude/RuFlo/Codex development context but remain outside the trading runtime.
- Generated advisory runtime state is still not repo state; `.claude-flow/data/`, logs, sessions, neural output, metrics, security JSON, `.ruflo/`, `.swarm/`, `.mcp.json`, local memory databases, daemon pid files, and similar tool-state files stay ignored or disposable
- `.ai/workflows/` now defines feature, security, release/PR, QA, performance, external-tooling, and generated-artifact-harvest agreements so future agents can use RuFlo/Claude-flow ideas without depending on generated runtime state
- `.ai/skills/ruflo-codex.instructions.md` and `.ai/playbooks/ruflo-advisory-checks.instructions.md` now document MCP-first plus global-CLI RuFlo usage for routing, diff risk, command risk, security, performance, coverage-gap, dependency, and code-boundary review; additional repo-local RuFlo init remains out of scope unless a temp smoke and explicit repo decision justify it
