...
- setup is still not yet a true one-command product onboarding flow: the next planned layer is `app:up` plus `app:setup/start/stop/update/doctor/uninstall`, preserving focused debug scripts while asking host-owned vs app-owned vs API/key-only vs skipped ownership for Ollama, Firecrawl, and Camofox before installing, starting, updating, or deleting anything
- `tools/camofox-browser` is still carrying npm-shaped setup/start hints even though the root repo is pnpm-owned; the planned migration is to use `pnpm --dir tools/camofox-browser install --ignore-scripts`, `pnpm --dir tools/camofox-browser run fetch:browser`, and `pnpm --dir tools/camofox-browser run test` while keeping browser binary fetch explicit and opt-in
