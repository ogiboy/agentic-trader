...
- setup is still not yet a fully interactive one-command product onboarding flow: the next layer should persist/display ownership decisions across setup-status and Web GUI readiness, preserve focused debug scripts, and keep asking host-owned vs app-owned vs API/key-only vs skipped ownership for Ollama, Firecrawl, and Camofox before installing, starting, updating, or deleting anything
