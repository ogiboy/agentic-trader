...
- setup is still not yet a true one-command product onboarding flow: the next planned layer is `app:up` plus `app:setup/start/stop/update/uninstall`, preserving focused debug scripts while asking host-owned vs app-owned vs API/key-only vs skipped ownership for Ollama, Firecrawl, and Camofox before installing, starting, updating, or deleting anything
