...
- `tools/camofox-browser` setup/start docs and root scripts now use `pnpm --dir tools/camofox-browser ...`; the npm lockfile can be removed only after a dedicated pnpm tool-root install/test smoke proves the migration is clean, and browser binary fetch remains explicit and opt-in
