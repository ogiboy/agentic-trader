...
- `research-flow-setup` reports the tracked `sidecars/research_flow/` workspace member path, Python version file, root lockfile/workspace environment presence, uv availability, and root setup/check/run commands; CrewAI is still not imported by core runtime modules or added to the root package dependency list
