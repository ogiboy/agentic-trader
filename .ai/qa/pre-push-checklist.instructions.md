...
the exact blocker and use the next strongest evidence without marking the tier
fully passed.
...
- WebGUI route negative tests: foreign origin, malformed JSON, oversized body,
  missing/invalid token when configured, token-protected browser unlock through
  the HttpOnly session cookie, rapid repeated runtime actions
