...
Use the checklist by risk tier rather than top-to-bottom for every change:
runtime/broker/security changes require all relevant sections; docs or prompt
changes require the baseline, affected surface, and repo-hygiene checks. If a
tool or surface is unavailable, record the exact skipped item and the fallback
evidence used.
...
- [ ] Run terminal smoke QA:
  - `pnpm run qa`
- [ ] For `.prompt.md`, `.agent.md`, or `.instructions.md` changes, run:
  - `pnpm run qa:prompts`
...
- [ ] Confirm `.ai/current-state.instructions.md`, `.ai/tasks.instructions.md`, and `.ai/decisions.instructions.md` are updated when architecture, runtime contracts, or assumptions change.
