...
If a required surface, command, token, model, provider, browser, or optional
helper is unavailable, record the skipped item and continue only with scenarios
that can still produce truthful evidence.
...
Recommended prompt/agent/instruction guidance check:

```bash
pnpm run qa:prompts
```

This deterministic check is the local source of truth when editor prompt
evaluation extensions are stale, quota-limited, or unavailable.
...
Branch push binary workflows should publish those identities as prerelease GitHub Releases as well as workflow artifacts. They must not edit `CHANGELOG.md`; tracked version files move only for explicit product-impacting branch version bumps or stable `main` release automation.

## Standard QA Workflow

1. Read `AGENTS.instructions.md`, `.ai/current-state.instructions.md`, `.ai/tasks.instructions.md`, `.ai/debugging.instructions.md`, and the QA scenario being executed.
...
6. Compare actual behavior against `.ai/qa/qa-checklist.instructions.md`.
