...
if [[ -z "${SONARQUBE_KEYCHAIN_ACCOUNT}" ]]; then
	echo "SONARQUBE_KEYCHAIN_ACCOUNT is required when USER is unavailable." >&2
	exit 1
fi
...
export SONARQUBE_TOKEN SONARQUBE_URL SONARQUBE_IDE_PORT
if [[ -n "${SONARQUBE_ORG:-}" ]]; then
