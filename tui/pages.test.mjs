...
function dashboardData() {
  return {
    ...
    calendar: {
      session: { is_tradable_now: true, state: 'open', timezone: 'America/New_York' },
    },
    canonicalAnalysis: {
      available: true,
      snapshot: {
...
describe('Ink TUI pages', () => {
  it('routes dashboard page keys to page components', () => {
    ...
    expect(getPageView({ page: 'overview', data, chat, instruction }).type).toBe(
      OverviewPage,
    );
    expect(getPageView({ page: 'runtime', data, chat, instruction }).type).toBe(
      RuntimePage,
    );
    expect(getPageView({ page: 'portfolio', data, chat, instruction }).type).toBe(
      PortfolioPage,
    );
    ...
    expect(getPageView({ page: 'settings', data, chat, instruction }).type).toBe(
      SettingsPage,
    );
  ...
  it('renders primary page content from complete dashboard data', () => {
    ...
    expect(renderText(PortfolioPage({ data }))).toContain('Cash (USD): 10000.00');
  ...
  it('renders fallback states for unavailable page data', () => {
    ...
    const unavailable = {
      ...
      portfolio: { ...data.portfolio, available: false, error: 'portfolio locked' },
      preferences: { ...data.preferences, available: false, error: 'prefs locked' },
