...
def _is_crypto_symbol(symbol: str) -> bool:
    upper = symbol.upper()
    return upper.endswith("-USD") or upper.endswith("-USDT") or upper in {"BTC", "ETH", "SOL", "BNB"}
...
def infer_market_session(
    *,
    ...
    if upper_symbol.endswith(".IS") or "BIST" in preferences.exchanges or "TR" in preferences.regions:
        local_dt = current_utc.astimezone(TR_TZ)
        state = _session_state_for_local_time(local_dt=local_dt, open_time=time(10, 0), close_time=time(18, 0))
        return MarketSessionStatus(
            symbol=symbol,
            venue="BIST",
            asset_class="equity",
    ...
    state = _session_state_for_local_time(local_dt=local_dt, open_time=time(9, 30), close_time=time(16, 0))
    return MarketSessionStatus(
        symbol=symbol,
        venue="US",
        asset_class="equity",
