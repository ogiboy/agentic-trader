...
- [ ] migrate optional Camofox helper setup from npm-owned commands to a pnpm-owned tool-root flow using `pnpm --dir tools/camofox-browser ...`, with browser binary fetch remaining explicit and opt-in
...
- Camofox helper setup commands now use `pnpm --dir tools/camofox-browser ...`; the npm lockfile can be removed in a later slice only after the pnpm tool-root install/test smoke is proven clean
...
- the next bootstrap slice should ask whether Ollama, Firecrawl, and Camofox are host-owned, app-owned, API/key-only, or skipped; app-owned services may start only through owner-tracked loopback service surfaces and must never kill host-owned processes
