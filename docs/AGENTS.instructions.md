...
- Treat `docs/` as a thin explanatory shell around the repo, not a second source
  of truth.
- Keep `docs/` and `webgui/` visually related, but do not invent a shared
  package before the surfaces stabilize.
- When behavior or workflow assumptions move, update the relevant docs page
  together with `.ai/current-state.instructions.md`,
  `.ai/tasks.instructions.md`, and `.ai/decisions.instructions.md`.
...
- GitHub Pages-compatible feedback flow that prepares browser-local GitHub issue
  drafts without server writes
