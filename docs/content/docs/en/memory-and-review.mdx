...
description:
  How trading memory, review evidence, and contributor notes stay separate and
  inspectable.
---
...
Memory is inspection support. It is not hidden permission to trade, and it
should not silently mutate broker, risk, or runtime policy.
...
should be written for operators first. Whenever a durable decision changes, the
right follow-up is usually:
...
The GitHub Pages docs build cannot write to the repository filesystem. Its
feedback widget stores a short browser-local draft and prepares a prefilled
GitHub issue link for manual submission.

If a future Node-hosted docs instance re-enables server-side feedback, it should
keep local auditability explicit and avoid pretending that external forwarding
