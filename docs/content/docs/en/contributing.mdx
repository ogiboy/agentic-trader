...
Only `main` mutates `pyproject.toml`, `CHANGELOG.md`, Git tags, and GitHub Releases automatically. Other branches run a version preview and may build test artifacts:

- `main`: stable SemVer tags such as `v0.9.5`
- `V1`, `V2`, and similar integration branches: `next` artifacts such as `v0.9.6-next.9870+gabc1234`
- feature branches: `beta` artifacts such as `v0.9.6-beta.9870+gabc1234`
...
- Binaries: PyInstaller macOS and Windows artifacts for branch testing, attached to the GitHub Release only for release tags
