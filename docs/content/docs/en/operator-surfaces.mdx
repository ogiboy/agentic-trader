...
description:
  Which surface to use for setup, paper runs, runtime monitoring, review, and
  troubleshooting.
---
...
No surface should invent a private runtime model. If a screen shows runtime,
broker, memory, review, or health state, that should come from shared contracts.
...
The docs app is also a surface. Its job is to explain the runtime and product
workflows, not to create a second runtime story.

That means:

- feedback copy must explain whether data is logged locally, forwarded to
  GitHub, or both
