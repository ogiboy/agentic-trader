...
description:
  One-shot runs, service mode, health visibility, and the operational truth
  every surface should share.
---
...
- `agentic_trader/workflows/service.py` owns continuous and background
  orchestration
- observer and UI surfaces should read shared runtime state instead of inferring
  it
...
The exact implementation can evolve, but docs and UI copy should respect those
...
Repeated requests to Ollama do not automatically mean the runtime is healthy. If
a cycle looks stuck, inspect:
