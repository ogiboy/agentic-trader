...
description:
  Install the local runtime, verify readiness, and open the control surfaces
...
Do this before debugging the Web GUI or Ink TUI. If `doctor` is already wrong,
...
The repo is still Ollama-oriented by default, but the direction is becoming more
provider-aware. Typical local model settings look like this:
...
Do not treat raw Ollama traffic as proof that the system is healthy. Model
reachability, runtime readiness, and recent logs should agree.
...
Terminal surfaces share the Python locale setting. Use
