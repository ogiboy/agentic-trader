...
title: Başlangıç
description:
  Lokal runtime'ı kur, hazır oluşu doğrula ve kontrol yüzeylerini hata
  gizlemeden aç.
---
...
- Araştırma helper'larını açıkça etkinleştiriyorsan isteğe bağlı Firecrawl ve
  Camofox
...
Bunu Web GUI ya da Ink TUI debug etmeden önce yap. `doctor` zaten yanlışsa UI
...
Depo varsayılan olarak hâlâ Ollama yönelimli, fakat yön giderek provider-aware
hâle geliyor. Tipik lokal model ayarları şöyle görünebilir:
...
Sadece Ollama trafiğini görmeyi sistemin sağlıklı olduğunun kanıtı sayma. Model
erişimi, runtime ready oluşu ve son loglar birbiriyle uyumlu olmalı.
...
Terminal yüzeyleri Python tarafındaki locale ayarını paylaşır.
