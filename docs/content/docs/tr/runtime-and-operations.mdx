...
lokal `paper`; `alpaca_paper` external paper prova yoludur ve credential,
paper endpoint ve açık enablement ister. V1 aktif al-sat yolu yine desteklenen
broker, manuel onay, audit, risk ve kill-switch kapılarından geçmelidir; gizli
live brokerage yolu yoktur.
...
source health'i açıklayabilir, fikirleri skorlayabilir ve broker-free proposal
candidate oluşturabilir; bir adayın pending proposal'a dönüşmesi için hâlâ
açık operator promotion adımı gerekir.
...
URL'inin bununla eşleşip eşleşmediğini bildirir. Stop akışı eski
app-managed portları temizlemeye çalışır; `11434` üzerinde kalan servis ise
host/default Ollama olarak görünür ve otomatik öldürülmez.
