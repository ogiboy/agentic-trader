...
`components/layout`, `lib/i18n` ve `lib/home/content` ayrımıyla uyguluyor. Web
GUI control room da aynı yöne gidiyor: üst seviye component state/render
coordinator olarak kalmalı; view dosyaları, shell chrome, dashboard polling,
action handler'ları, request helper'ları, primitive'ler ve typed dil bazlı
İngilizce/Türkçe copy ayrı modüllerde durmalı.
...
`context-formatting`, loading panelleri, odaklı view modülleri, shell chrome,
primitive'ler ve typed İngilizce/Türkçe copy modüllerini içeriyor.
