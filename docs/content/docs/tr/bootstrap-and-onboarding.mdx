...
Firecrawl internal SDK/API-key yolunu önce kullanmalı; authenticated host CLI fallback yalnızca Firecrawl host-owned kaydedildiğinde çalışmalı, app-owned/API-only/skipped modlarında bu fallback kapalı ve görünür kalmalı.
...
`app:start` aktif provider `ollama` ise önceden yapılandırılmış app-owned model service'i başlatabilir; ama binary kurmayı, model çekmeyi, host listener'ı sahiplenmeyi veya açıkça seçilmiş OpenAI-compatible adaptörü override etmeyi sessizce yapmamalı.
