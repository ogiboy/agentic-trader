 ...
 * flag server.js for env-harvesting (env + network in same file).
 */
...
/** Compatibility shim for upstream reporter config; Agentic Trader keeps it local. */
...
function loadConfig() {
  return {
    ...
    host: process.env.CAMOFOX_HOST || process.env.HOST || '127.0.0.1',
    ...
    serverEnv: {
      ...
      CAMOFOX_BROWSER_PREWARM: process.env.CAMOFOX_BROWSER_PREWARM,
