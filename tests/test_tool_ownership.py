...
def test_ownership_note_returns_generic_app_owned_for_non_firecrawl() -> None:
    ...
    assert (
        "app may manage" in note_lower
        or "explicit lifecycle" in note_lower
        or "app-owned" in note_lower
    )
...
def test_ownership_mode_for_tool_returns_undecided_without_file(
    tmp_path: Path,
) -> None:
    settings = _settings(tmp_path)
