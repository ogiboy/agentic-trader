...
Covers _parse_ui_locale(), _ui_payload(), and _upsert_env_local_value()
from agentic_trader.cli, plus the UI_LOCALE_ENV constant.
"""
...
def test_parse_ui_locale_returns_none_for_none_input() -> None:
    assert _parse_ui_locale(None) is None
...
def test_parse_ui_locale_accepts_all_supported_locales() -> None:
    for locale in SUPPORTED_UI_LOCALES:
        result = _parse_ui_locale(locale)
...
def test_upsert_env_local_value_creates_file_if_missing(
    ...
    _upsert_env_local_value("AGENTIC_TRADER_UI_LOCALE", "tr")
...
def test_upsert_env_local_value_writes_without_quotes(
    ...
    _upsert_env_local_value("SOME_KEY", "some_value")
...
def test_upsert_env_local_value_preserves_other_keys(
    ...
    _upsert_env_local_value("NEW_KEY", "new_value")
