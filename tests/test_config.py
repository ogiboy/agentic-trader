...
def test_llm_provider_reads_openai_compatible_from_env(monkeypatch) -> None:
    monkeypatch.setenv("AGENTIC_TRADER_LLM_PROVIDER", "openai-compatible")

    assert Settings().llm_provider == "openai-compatible"


def test_llm_provider_can_be_set_directly() -> None:
    settings = Settings(llm_provider="openai-compatible")

    assert settings.llm_provider == "openai-compatible"


def test_openai_compatible_api_key_defaults_to_none(monkeypatch) -> None:
...
def test_openai_compatible_api_key_reads_canonical_env(monkeypatch) -> None:
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.setenv("AGENTIC_TRADER_OPENAI_COMPATIBLE_API_KEY", "my-local-token")

    assert Settings().openai_compatible_api_key == "my-local-token"


def test_openai_compatible_api_key_reads_openai_api_key_alias(monkeypatch) -> None:
    monkeypatch.delenv("AGENTIC_TRADER_OPENAI_COMPATIBLE_API_KEY", raising=False)
    monkeypatch.setenv("OPENAI_API_KEY", "openai-alias-token")

    assert Settings().openai_compatible_api_key == "openai-alias-token"
